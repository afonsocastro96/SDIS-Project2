...
public class TestApp {
    ...
    public String execute(String subProtocol, String opnd1, String opnd2) throws RemoteException {
        switch (subProtocol) {
            case "BACKUP":
                if (opnd2 == null)
                    return "BACKUP outdated.subprotocol has 2 operands";
                return backup(opnd1, opnd2);
            case "BACKUPENH":
                if (opnd2 == null)
                    return "BACKUPENH outdated.subprotocol has 2 operands";
                return backupEnh(opnd1, opnd2);
            case "RESTORE":
                if (opnd2 != null)
                    return "RESTORE outdated.subprotocol only has 1 operand";
                return restore(opnd1);
            case "RESTOREENH":
                if (opnd2 != null)
                    return "RESTOREENH outdated.subprotocol only has 1 operand";
                return restoreEnh(opnd1);
            case "DELETE":
                if (opnd2 != null)
                    return "DELETE outdated.subprotocol only has 1 operand";
                return delete(opnd1);
            case "DELETEENH":
                if (opnd2 != null)
                    return "DELETEENH outdated.subprotocol only has 1 operand";
                return deleteEnh(opnd1);
            case "RECLAIM":
                if (opnd2 != null)
                    return "RECLAIM outdated.subprotocol only has 1 operand";
                return reclaim(opnd1);
            case "RECLAIMENH":
                if (opnd2 != null)
                    return "RECLAIMENH outdated.subprotocol only has 1 operand";
                return reclaimEnh(opnd1);
        }
        return "Unknown outdated.subprotocol: " + subProtocol;
    }
    ...
    public static void main(String [] args) {
        if (! ((MIN_ARGS <= args.length) && (args.length <= MAX_ARGS))) {
            System.err.println("Usage: java outdated.TestApp <peer_ap> <sub_protocol> <opnd_1> <opnd_2>");
            return;
        }
