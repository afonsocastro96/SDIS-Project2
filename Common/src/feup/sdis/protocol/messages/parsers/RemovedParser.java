...
import feup.sdis.protocol.messages.RemovedMessage;
 ...
 * Removed parser
 */
public class RemovedParser extends ProtocolParser {
    ...
    public ProtocolMessage parse(byte[] message) throws MalformedMessageException {
        ...
        if (header.size() != 5)
            throw new MalformedMessageException("Wrong number of arguments for the REMOVED message: 5 arguments must be present");

        if (!header.get(0).equalsIgnoreCase(ProtocolMessage.Type.REMOVED.toString()))
        ...
        return new RemovedMessage(UUID.fromString(header.get(2)), UUID.fromString(header.get(3)), Integer.parseInt(header.get(4)));
    }
}
