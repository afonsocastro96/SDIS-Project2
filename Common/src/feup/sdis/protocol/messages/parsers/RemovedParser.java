...
public class RemovedParser extends ProtocolParser {
    ...
    public ProtocolMessage parse(byte[] message) throws MalformedMessageException {
        ...
        if (header.size() != 4)
            throw new MalformedMessageException("Wrong number of arguments for the REMOVED message: 4 arguments must be present");
        ...
        return new RemovedMessage(UUID.fromString(header.get(2)), Integer.parseInt(header.get(3)));
    }
}
