...
public class GetChunkParser extends ProtocolParser {
    ...
    public ProtocolMessage parse(byte[] message) throws MalformedMessageException {
        ...
        if (header.size() != 5)
            throw new MalformedMessageException("Wrong number of arguments for the GETCHUNK message: 5 arguments must be present");
        ...
        return new GetChunkMessage(UUID.fromString(header.get(2)), UUID.fromString(header.get(3)), Integer.parseInt(header.get(4)));
    }
}
